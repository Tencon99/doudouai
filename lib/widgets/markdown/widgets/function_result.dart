...
import 'tag.dart';
import 'package:doudouai/widgets/expandable_widget.dart';
...
SpanNodeGeneratorWithTag functionResultGenerator =
    SpanNodeGeneratorWithTag(tag: _functionResultTag, generator: (e, config, visitor) => FunctionResultNode(e.attributes, e.textContent, config));
    ...
    return ExpandableWidget(
      ...
      header: ExpandableRow(
        ...
        children: [
          ...
          Gap(size: 4),
          Expanded(
            child: Text("${widget.attributes['name']} result: ${widget.textContent}",
                maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: AppColors.getFunctionTextColor(context))),
          ),
          if (!widget.isClosed)
            SizedBox(
              width: 12,
